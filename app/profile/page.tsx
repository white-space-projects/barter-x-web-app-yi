 ...
 * - Address (mandatory for profile completion)
 * - Shipping Addresses per Offer
...
import { getCountryNames, getCitiesForCountry, getCountryCode } from "@/lib/countries-data";
...
import type { ProfileAddress, NotificationPreferences, Offer, ProductType } from "@/lib/types";

export default function ProfilePage() {
  const { auth, authReady, logout, getMyOffers, updateOffer, getUnreadCount, getTotalUnreadMessages } = useBarterStore();
  const router = useRouter();
  
  // Navigation state (used when sidebar/bottom nav is shown)
  const [activeProductType, setActiveProductType] = useState<ProductType>("cross-product");
  
  // Calculate total unread for badge
  const totalUnread = getUnreadCount() + getTotalUnreadMessages();
  ...
  // Filter offers - active first, then optionally show inactive
  const displayedOffers = useMemo(() => {
    // For now, all offers are "active" - in real app, filter by status
    return myOffers;
  }, [myOffers, showInactiveOffers]);
  ...
  // Check if this is a new user (needs to complete profile)
  ...
  useEffect(() => {
    if (auth.user) {
      ...
      // Load profile address if exists
  ...
  // Load offer shipping addresses - use offer IDs as dependency to prevent infinite loop
  ...
  useEffect(() => {
    ...
    offers.forEach((offer) => {
      // Skip if we already have this offer's address
  ...
  async function handleSaveProfile() {
    ...
    // In real app, this would call POST /api/profile/save
    // For now, we just show success and redirect
    toast.success("Profile saved successfully");
    setSaving(false);

    // If new user, redirect to workspace
    if (isNewUser) {
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1">
        ...
        {showDesktopNav && (
          <SidebarNav
            ...
            onSelectUtilityTab={(tab) => {
              if (tab) router.push("/workspace");
            }}
            onAddOffer={() => router.push("/workspace")}
        ...
        <div className="space-y-6">
          {/* Basic Info Section - includes address fields */}
          {activeSection === "basic-info" && (
            <section className="rounded-xl border border-border bg-card p-5">
              ...
              {/* Address Section - merged into Basic Info */}
              ...
              {/* Address Line 2 - only visible if Line 1 is filled */}
                {addressLine1 && (
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-foreground">
                      ...
                      >
                        ...
                        {isExpanded && offerAddr && (
                          <div className="border-t border-border p-4 bg-secondary/30">
                            ...
                            {/* Address fields - disabled if same as profile */}
